...
var Chp = (function() {
    ...
    function draw(ctx, cur, max, yOffset) {
        ...
        ctx.drawImage(hpText, 294, 505);
        document.getElementById('cvs').style.letterSpacing = '0px';
        ctx.fillStyle = "#F00";
        ctx.fillRect(325, 500, Math.floor(max * 1.25) / Math.ceil(max / 150), 21);
        ...
        ctx.fillRect(325, 500, Math.floor(cur * 1.25) / Math.ceil(max / 150), 21);
        // Karma purple overlay
        if (Player.getKarma() > 0) {
            var karmaWidth = Math.min(Player.getKarma() * 2, Math.floor(cur * 1.25) / Math.ceil(max / 150));
            ctx.fillStyle = "#808";
            ctx.globalAlpha = 0.7;
            ctx.fillRect(325 + Math.floor(cur * 1.25) / Math.ceil(max / 150) - karmaWidth, 500, karmaWidth, 21);
            ctx.globalAlpha = 1;
        }
        ctx.font = "24px Mars Needs Cunnilingus";
        ctx.fillStyle = "#FFF";
        ctx.fillText(plyrName + "   lv " + lv, 81, 518);
        ctx.fillText(Math.ceil(cur) + " / " + max, 339 + Math.floor(max * 1.25) / Math.ceil(max / 150), 518);
        ctx.restore();
    }
